...
void perform_custom_space_cadet(keyrecord_t *record, uint16_t sc_keycode, uint8_t holdMod, uint16_t holdKey, uint8_t tapMod, uint16_t keycode) {
    ...
    if (record->event.pressed) {
        ...
        if (layer < 8 && !IS_LAYER_ON(layer)) {
            layer_on(layer);
        }
        if CustomRegisterCodes (holdKey) {
...
bool process_custom_space_cadet(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_EASYHOME: {
            perform_custom_space_cadet(record, keycode, CHOEA_KEYS);
            return false;
        }
        case KC_EASYEND: {
            perform_custom_space_cadet(record, keycode, CENEA_KEYS);
            return false;
        }
        case KC_EASYSHIFT: {
        ...
        case KC_EASYTAB: {
            perform_custom_space_cadet(record, keycode, CTAEA_KEYS);
            return false;
        }
        default: {
            if (record->event.pressed) {
