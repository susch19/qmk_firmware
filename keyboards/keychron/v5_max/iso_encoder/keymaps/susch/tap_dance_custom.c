...
void betternum_finished(tap_dance_state_t  *state, void *user_data) {
...
void betternum_reset(tap_dance_state_t  *state, void *user_data) {
...
void easyshift_finished(tap_dance_state_t  *state, void *user_data) {
    xtap_state.state = cur_dance(state);
    keyrecord_t pressed = {{{0,3},true,timer_read()}, {0,0,0,0,0}};
    switch (xtap_state.state) {
        case TD_SINGLE_TAP:
            register_code(KC_CAPS_LOCK);
            sendKeyCodeOverRawHid(KC_CAPS_LOCK, &pressed);
        break;
        case TD_SINGLE_HOLD:
...
void easyshift_reset(tap_dance_state_t  *state, void *user_data){
    keyrecord_t depressed = {{{0,3},false,timer_read()}, {0,0,0,0,0}};
    switch (xtap_state.state) {
        case TD_SINGLE_TAP:
            unregister_code(KC_CAPS_LOCK);
            sendKeyCodeOverRawHid(KC_CAPS_LOCK, &depressed);
            break;
        case TD_SINGLE_HOLD:
...
void staronhold_finished(tap_dance_state_t  *state, void *user_data) {
    xtap_state.state = cur_dance(state);
...
void staronhold_reset(tap_dance_state_t  *state, void *user_data) {
